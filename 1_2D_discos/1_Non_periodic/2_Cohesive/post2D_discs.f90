...
type  ::  T_CORPS
   ...
   real(kind=8),allocatable,dimension(:,:)  ::  vertex,vertex_ref ! For the vtk visualization
end type T_CORPS
...
type(T_CORPS),allocatable,dimension(:)      ::  TAB_DISC,TAB_PLAN
...
integer                                    ::  calcul_orientation=0, calcul_construct_bodies=0, &
                                               c_draw = 0, c_n_ctc_probability = 0

!Variables DavidC.
integer                                    ::  nb_part_invi, nb_part_visi
integer                                    ::  N_DKDKx, nc_disc
integer                                    ::  N_DKJCx, nc_wall
...
SUBROUTINE read_Vloc_dof_bodies
    ...
    do
      ...
      if (text == '$bdyty') then
        ...
        if (i<n_particles+1) then
          ...
          TAB_DISC(i)%behav = 'PLEXx'
          read(2,'(29X, 3(5x,D14.7,2X))') center1,center2,center3
          
          TAB_DISC(i)%radius = radius
          TAB_DISC(i)%center_ref(1)=center1
          TAB_DISC(i)%center_ref(2)=center2
          TAB_DISC(i)%center_ref(3)=center3
          read(2,*)
          read(2,*)
          read(2,*)
          read(2,*)
          ...
          TAB_DISC(i)%Area = pi*(radius**2)
       
          TAB_DISC(i)%lieu=0
          TAB_DISC(i)%center(:)=0.D0
    ...
    do
      ...
      if (text == '$bdyty') then
        ...
        if (i<n_particles+1) then
          ...
          TAB_DISC(i)%center(1)=coor1+TAB_DISC(i)%center_ref(1)
          TAB_DISC(i)%center(2)=coor2+TAB_DISC(i)%center_ref(2)
          TAB_DISC(i)%center(3)=coor3+TAB_DISC(i)%center_ref(3)
  ...
  if (err/=0) then
    ...
    allocate(TAB_CONTACT(N_DKDKx+N_DKJCx))
    
    ! Storing the contacts information
    ! In the array the disc-disc contacts are stored first
    nb_ligneCONTACT=0
    nc_disc = 0
    nc_wall = 0
    ...
    do 
      ...
      if (text2 == '$icdan  DKDKx') then
        nc_disc = nc_disc + 1
        read(4,*)
        read(4,'(8X,i5,30X,i5)') icdent,ianent
        ...
        TAB_CONTACT(nc_disc)%icdent=icdent
        TAB_CONTACT(nc_disc)%ianent=ianent
        TAB_CONTACT(nc_disc)%n(1)=n1
        TAB_CONTACT(nc_disc)%n(2)=n2
        TAB_CONTACT(nc_disc)%n(3)=n3
        TAB_CONTACT(nc_disc)%t(1)=n2
        TAB_CONTACT(nc_disc)%t(2)=-n1
        TAB_CONTACT(nc_disc)%t(3)=0.D0
        TAB_CONTACT(nc_disc)%coor_ctc(1)=coor_ctc1
        TAB_CONTACT(nc_disc)%coor_ctc(2)=coor_ctc2
        TAB_CONTACT(nc_disc)%coor_ctc(3)=coor_ctc3
        TAB_CONTACT(nc_disc)%rn=rn
        TAB_CONTACT(nc_disc)%rt=rt
        TAB_CONTACT(nc_disc)%rs=rs
        TAB_CONTACT(nc_disc)%nature='DKDKx'
        TAB_CONTACT(nc_disc)%type  = 0
      end if
      ...
      if (text2 == '$icdan  DKJCx') then
        ...
        TAB_CONTACT(N_DKDKx+nc_wall)%icdent=icdent
        TAB_CONTACT(N_DKDKx+nc_wall)%ianent=ianent
        TAB_CONTACT(N_DKDKx+nc_wall)%n(1)=n1
        TAB_CONTACT(N_DKDKx+nc_wall)%n(2)=n2
        TAB_CONTACT(N_DKDKx+nc_wall)%n(3)=n3
        TAB_CONTACT(N_DKDKx+nc_wall)%t(1)=n2
        TAB_CONTACT(N_DKDKx+nc_wall)%t(2)=-n1
        TAB_CONTACT(N_DKDKx+nc_wall)%t(3)=0.D0
        TAB_CONTACT(N_DKDKx+nc_wall)%coor_ctc(1)=coor_ctc1
        TAB_CONTACT(N_DKDKx+nc_wall)%coor_ctc(2)=coor_ctc2
        TAB_CONTACT(N_DKDKx+nc_wall)%coor_ctc(3)=coor_ctc3
        TAB_CONTACT(N_DKDKx+nc_wall)%rn=rn
        TAB_CONTACT(N_DKDKx+nc_wall)%rt=rt
        TAB_CONTACT(N_DKDKx+nc_wall)%rs=rs
        TAB_CONTACT(N_DKDKx+nc_wall)%nature='DKJCx'
        TAB_CONTACT(N_DKDKx+nc_wall)%type  = 0
      end if
      
      if (nc_disc+nc_wall == N_DKDKx + N_DKJCx) then
        exit
      endif
    end do
...
subroutine compacity
  ...
  do i=1,n_particles
    if (TAB_DISC(i)%in_box) then
      Area_grain = Area_grain + TAB_DISC(i)%Area
    end if
  end do
...
subroutine qoverp
  ...
  do i=1,nb_ligneCONTACT
    if (TAB_CONTACT(i)%in_box) then
      if  (TAB_CONTACT(i)%nature == 'DKJCx') cycle
      ...
      Lik(1) = TAB_DISC(cd)%center(1)-TAB_DISC(an)%center(1)
      Lik(2) = TAB_DISC(cd)%center(2)-TAB_DISC(an)%center(2)
...
subroutine nb_coordination
  ...
  do i=1,n_particles
    if (TAB_DISC(i)%in_box) zp=zp+1
  end do
  
  ! Computing the number of contacts
  do i=1,nb_ligneCONTACT
    ! Only between particles
    if (TAB_CONTACT(i)%nature /= 'DKDKx') cycle
    ! Contacts in the box
    if (TAB_CONTACT(i)%in_box) then
      zc=zc+1
      ! The contacts in compression
      if (TAB_CONTACT(i)%rn .ge. 0.D0 ) then
        z_c_compresive = z_c_compresive + 1
      ! The contacts in tension
      else
...
subroutine walls_force
  ...
  do i=0, nb_ligneCONTACT
    if (TAB_CONTACT(i)%nature== 'DKJCx') then
      if(TAB_CONTACT(i)%ianent == idw1) then
        fn1 = fn1 + abs(TAB_CONTACT(i)%rn)
        ft1 = ft1 + abs(TAB_CONTACT(i)%rt)
        
      elseif(TAB_CONTACT(i)%ianent == idw2) then
        fn2 = fn2 + abs(TAB_CONTACT(i)%rn)
        ft2 = ft2 + abs(TAB_CONTACT(i)%rt)
        
      elseif(TAB_CONTACT(i)%ianent == idw3) then
        fn3 = fn3 + abs(TAB_CONTACT(i)%rn)
        ft3 = ft3 + abs(TAB_CONTACT(i)%rt)
        
      elseif(TAB_CONTACT(i)%ianent == idw4) then
        fn4 = fn4 + abs(TAB_CONTACT(i)%rn)
        ft4 = ft4 + abs(TAB_CONTACT(i)%rt)
...
subroutine draw
  ...
  integer                                        ::  l_cdt, l_ant, n_l_sides
  real(kind=8), dimension(2)                     ::  cur_disc_vertex
  real(kind=8)                                   ::  cur_angle_rad
  ...
  n_l_sides = 16
  n_l_vertices = n_particles * n_l_sides
  ...
  do i=1, n_particles + n_walls
    ...
    if (i .le. n_particles) then
      do j=1, n_l_sides
        ...
        cur_disc_vertex(1) = TAB_DISC(i)%center(1) + TAB_DISC(i)%radius*cos(cur_angle_rad)
        cur_disc_vertex(2) = TAB_DISC(i)%center(2) + TAB_DISC(i)%radius*sin(cur_angle_rad)
        k = k + 1
        
        if(k .lt. 3) then 
          if (cur_disc_vertex(1) .lt. 0) then
            write(121,'(F11.8,A)', advance = 'no') cur_disc_vertex(1), ' '
          else
            write(121,'(F9.7,A)', advance = 'no') cur_disc_vertex(1), ' '
          end if
          
          if (cur_disc_vertex(2) .lt. 0) then
            write(121,'(F11.8,A)', advance = 'no') cur_disc_vertex(2), ' '
          else 
            write(121,'(F9.7,A)', advance = 'no') cur_disc_vertex(2), ' '
          end if
          
          write(121,'(F11.8,A)', advance = 'no') 0.D0 , ' '
    ...
    do j=1, n_l_sides
      ...
      if (j .lt. n_l_sides) then
        if (curr_l_faces - 1 + j .lt. 10) then
          ...
          write(121, '(I6)', advance = 'no') curr_l_faces - 1 + j
        end if
  ...
  do i=1, n_particles + n_walls
    ! For the particles
    if (i .le. n_particles) then
      curr_l_vector(:) = TAB_DISC(i)%center(:) - TAB_DISC(i)%center_ref
      write(121, '(3(F12.9,A))') curr_l_vector(1), ' ', curr_l_vector(2), ' ', 0.D0
    else
    !For the walls 
  ...
  do i=1, n_particles + n_walls
    if (i .le. n_particles) then
      curr_l_vector(1) = TAB_DISC(i)%V(1)
      curr_l_vector(2) = TAB_DISC(i)%V(2)
      curr_l_vector(3) = TAB_DISC(i)%V(3)
  ...
  do i=1, n_particles + n_walls
    ...
    if (i .le. n_particles) then
      ...
      do j=1, N_DKDKx
        l_cdt = TAB_CONTACT(j)%icdent
        l_ant = TAB_CONTACT(j)%ianent
        
        if(TAB_DISC(l_cdt)%behav /= 'PLEXx' .or. TAB_DISC(l_ant)%behav /= 'PLEXx') cycle
    ...
    if (i .le. n_particles) then
      ...
      do j=1, N_DKDKx
        ...
        if(l_cdt == i .or. l_ant == i) then
          ...
          !if (TAB_CONTACT(j)%rn .gt. 0.0) then
          l_counter = l_counter + 1
          !end if
        end if
      end do
  ...
  do i = 1, N_DKDKx
    if (TAB_CONTACT(i)%nature /= 'DKDKx') cycle
    vtk_ave_force = vtk_ave_force + abs(TAB_CONTACT(i)%rn)
  end do
  
  vtk_ave_force = vtk_ave_force/N_DKDKx
  
  ! Force scale parameter
  l_force_scale = 0.D0
  do i=1, n_particles
    l_force_scale = l_force_scale + TAB_DISC(i)%radius
  end do
  
  ! The scale is here!
  ! 7.5% of average radius
  l_force_scale = (l_force_scale/n_particles)*0.075
  ...
  do i=1, N_DKDKx
    ...
    vtk_cd_center(:) = TAB_DISC(l_cdt)%center(:)
    vtk_an_center(:) = TAB_DISC(l_ant)%center(:)
  ...
  do i=1, N_DKDKx
    if (TAB_CONTACT(i)%nature /= 'DKDKx') cycle
    write(121, '(F15.7)') TAB_CONTACT(i)%rn
  end do
  ...
  do i=1, N_DKDKx
    if (TAB_CONTACT(i)%nature /= 'DKDKx') cycle
    write(121, '(F15.7)') TAB_CONTACT(i)%rt
  end do
  ...
  do i=1, N_DKDKx
    if(TAB_CONTACT(i)%nature /= 'DKDKx') cycle
    !if(TAB_CONTACT(i)%rn .le. 0.D0) cycle
  ...
  do i = 1, n_particles
    vtk_ave_rad = vtk_ave_rad + TAB_DISC(i)%radius
  end do
...
subroutine contact_direction
  ...
  do i=1, ninter
    ...
    do j=1,nb_ligneCONTACT
      ! Only between discs
      if(TAB_CONTACT(j)%nature == 'DKJCx') cycle
      ! All contacts
...
!==============================================================================
  ...
  implicit none
    ...
    do i=1, n_particles
      if (TAB_DISC(i)%in_box) then
        vipar = vipar + 1
        if (TAB_DISC(i)%radius<rmin) then
          rmin = TAB_DISC(i)%radius
        endif 
        if (TAB_DISC(i)%radius>rmax) then
	       rmax = TAB_DISC(i)%radius
        endif
      end if
    end do    
    ...
    do j=1, until		
      do i=1, n_particles
        if (TAB_DISC(i)%in_box) then
	       if(TAB_DISC(i)%radius .ge. (rmin + (j-1)*interval) .and. TAB_DISC(i)%radius .lt. (interval*j + rmin)) then 
	         cant(j) = cant(j) + 1
	       endif
        end if
      end do
...
subroutine anisotropy_contact
  ...
  real(kind=8),dimension(2)                :: wr , wi, wr_c , wi_c, wr_t, wi_t
  real(kind=8)                             :: F1, F2, cpt, cpt_c, cpt_t, Rnik, F1_c, F2_c, F1_t, F2_t
  ...
  do i=1, nb_ligneCONTACT
    ...
    if (TAB_CONTACT(i)%in_box) then
      ! Checking it is a contact between two discs
      if (TAB_CONTACT(i)%nature == 'DKJCx') cycle
      cd     = TAB_CONTACT(i)%icdent
      an     = TAB_CONTACT(i)%ianent
      nik(1) = TAB_CONTACT(i)%n(1)
      ...
      ! Decomposition in tensile and compresive tensors
      if (Rnik .ge. 0) then
  ...
  ! Finding the eigen values for the full fabric tensor and the decompositions
  lda  = 2
  matz = 1
  matz_c = 1
  matz_t = 1
  ...
  F1_c = max(wr_c(1),wr_c(2))
  F2_c = min(wr_c(1),wr_c(2))
  ...
  F1_t = max(wr_t(1),wr_t(2))
  F2_t = min(wr_t(1),wr_t(2))
  ...
  write(104,'(4(1X,D12.5))') time, 2*(F1-F2), 2*(F1_c-F2_c)*cos(2*(ang_f-ang_fc)), 2*(F1_t-F2_t)*cos(2*(ang_f-ang_ft))
...
!==============================================================================
! Calcul de l'anisotropie de forces
!==============================================================================
subroutine anisotropy_force
  ...
  real(kind=8), dimension(2,2)                 :: Fabric_N_c, Fabric_N_t
  real(kind=8), dimension(2,2)                 :: Fabric_T_c, Fabric_T_t
  real(kind=8), dimension(2,2)                 :: Fabric_F_c, Fabric_F_t
  real(kind=8), dimension(2)                   :: nik,tik
  integer                                      :: i,cd,an
  integer                                      :: ierror,matz,lda
  real(kind=8), dimension(2)                   :: wrn, win, wrf, wif, wrtemp, witemp
  real(kind=8)                                 :: X1n, X2n, X1f, X2f, cpt, Rtik, Rnik, av_force
  real(kind=8)                                 :: X1n_c, X2n_c, X1n_t, X2n_t, X1f_c, X2f_c,  X1f_t, X2f_t
  ...
  do i=1,nb_ligneCONTACT
    if (TAB_CONTACT(i)%in_box) then
      ...
      ! Building the decomposition in tensile and compresive forces
      if (Rnik .ge. 0) then
        Fabric_N_c(1,1:2) = Rnik*nik(1)*nik(1:2) + Fabric_N_c(1,1:2)
        Fabric_N_c(2,1:2) = Rnik*nik(2)*nik(1:2) + Fabric_N_c(2,1:2)
        
        Fabric_T_c(1,1:2) = Rtik*nik(1)*tik(1:2) + Fabric_T_c(1,1:2)
        Fabric_T_c(2,1:2) = Rtik*nik(2)*tik(1:2) + Fabric_T_c(2,1:2)
      else
        Fabric_N_t(1,1:2) = Rnik*nik(1)*nik(1:2) + Fabric_N_t(1,1:2)
        Fabric_N_t(2,1:2) = Rnik*nik(2)*nik(1:2) + Fabric_N_t(2,1:2)
        
        Fabric_T_t(1,1:2) = Rtik*nik(1)*tik(1:2) + Fabric_T_t(1,1:2)
        Fabric_T_t(2,1:2) = Rtik*nik(2)*tik(1:2) + Fabric_T_t(2,1:2)
      end if
    end if
  end do
  ...
  ! oOrmalizing the decomposed tensors
  Fabric_N_c = Fabric_N_c/(cpt*av_force)
  Fabric_N_t = Fabric_N_t/(cpt*av_force)
  Fabric_T_c = Fabric_T_c/(cpt*av_force)
  Fabric_T_t = Fabric_T_t/(cpt*av_force)
  
  ! Building the full fabric forces tensor (Built this always before using rg!!!)
  Fabric_F = Fabric_N + Fabric_T
  Fabric_F_c = Fabric_N_c + Fabric_T_c
  Fabric_F_t = Fabric_N_t + Fabric_T_t
  
  ! Finding the eigen values of the fabric normal forces tensor and the decompositions
  lda = 2
  matz = 1
  
  ! The normal forces tensor
  call rg(lda, 2, Fabric_N, wrn, win, matz, localframeN, ierror)
  X1n = max(wrn(1),wrn(2))
  X2n = min(wrn(1),wrn(2))
  
  ! The full forces tensor
  call rg(lda, 2, Fabric_F, wrf, wif, matz, localframeF, ierror)
  X1f = max(wrf(1),wrf(2))
  X2f = min(wrf(1),wrf(2))
  
  ! The normal forces tensor in compression
  call rg(lda, 2, Fabric_N_c, wrtemp, witemp, matz, localframeN, ierror)
  X1n_c = max(wrtemp(1),wrtemp(2))
  X2n_c = min(wrtemp(1),wrtemp(2))
  
  ! The normal forces tensor in tension
  call rg(lda, 2, Fabric_N_t, wrtemp, witemp, matz, localframeN, ierror)
  X1n_t = max(wrtemp(1),wrtemp(2))
  X2n_t = min(wrtemp(1),wrtemp(2))
  
  ! The full tensor in compression
  call rg(lda, 2, Fabric_F_c, wrtemp, witemp, matz, localframeN, ierror)
  X1f_c = max(wrtemp(1),wrtemp(2))
  X2f_c = min(wrtemp(1),wrtemp(2))
  
  ! The full tensor in tension
  call rg(lda, 2, Fabric_F_t, wrtemp, witemp, matz, localframeN, ierror)
  X1f_t = max(wrtemp(1),wrtemp(2))
  X2f_t = min(wrtemp(1),wrtemp(2))
  
  if (first_over_all) then
     write(105,*) '#   time     ', '     afn     ', '     aft     ', '    afn_c    ', '    afn_t    ', &
                  '    aft_c    ', '    aft_t    '
  end if 
  
  write(105,'(7(1X,D12.5))') time, 2*(X1n-X2n)/(X1n+X2n),2*(X1f-X2f)/(X1f+X2f), &
                                   2*(X1n_c-X2n_c)/(X1n_c+X2n_c), 2*(X1n_t-X2n_t)/(X1n_t+X2n_t), &
                                   2*(X1f_c-X2f_c)/(X1f_c+X2f_c), 2*(X1f_t-X2f_t)/(X1f_t+X2f_t)
...
!==============================================================================
  ...
  implicit none
    ...
    do i=1,n_particles
      if (TAB_DISC(i)%in_box) then
        V  = V  + sqrt( TAB_DISC(i)%V(1)**2 + TAB_DISC(i)%V(2)**2 )
        Vx = Vx +       TAB_DISC(i)%V(1)
        Vy = Vy +       TAB_DISC(i)%V(2)
        Vr = Vr +       TAB_DISC(i)%V(3)
        cpt = cpt +1
      end if
    end do
...
!==============================================================================
! Testing that particles and contacts are indeed within the box
!==============================================================================
...
subroutine test_position_contact
  ...
  do i=1,n_particles
    if ((TAB_DISC(i)%center(1) .gt. xmin).and.(TAB_DISC(i)%center(1) .lt. xmax)) then
      if ((TAB_DISC(i)%center(2) .gt. ymin).and.(TAB_DISC(i)%center(2) .lt. ymax)) then
        TAB_DISC(i)%in_box = .true.
        nb_part_visi = nb_part_visi + 1
      else
        TAB_DISC(i)%in_box = .false.
      end if
    else
      TAB_DISC(i)%in_box = .false.
    end if
  ...
  do i=1, N_DKDKx
    if(TAB_CONTACT(i)%coor_ctc(1) .gt. xmin .and. TAB_CONTACT(i)%coor_ctc(1) .lt. xmax) then
      if(TAB_CONTACT(i)%coor_ctc(2) .gt. ymin .and. TAB_CONTACT(i)%coor_ctc(2) .lt. ymax) then
        TAB_CONTACT(i)%in_box = .true.
      else
        TAB_CONTACT(i)%in_box = .false.
      endif
    else
      TAB_CONTACT(i)%in_box = .false.
    endif
...
dmin = 0.D0
do i=1,n_particles
  ymax = max( ymax, TAB_DISC(i)%center(2) )
  ymin = min( ymin, TAB_DISC(i)%center(2) )
  xmax = max( xmax, TAB_DISC(i)%center(1) )
  xmin = min( xmin, TAB_DISC(i)%center(1) )
  dmin = dmin + 2*TAB_DISC(i)%radius
end do
...
do i=1,n_particles
  TAB_DISC(i)%behav = 'plexx'
  TAB_DISC(i)%radius = 0.D0
  if ( TAB_DISC(i)%center(2) < 2*dmin ) TAB_DISC(i)%behav='wallb'
  if ( TAB_DISC(i)%center(2) > ymax - 3*dmin ) TAB_DISC(i)%behav='wallh'
end do

cpt = 0
do i=1,n_particles
  if (TAB_DISC(i)%behav /= 'plexx')  cycle
  ...
  write(1004,'(1X,A5,2X,I5,2X,A5,2X,A5,2(2X,A5,D14.7))') &
       'PLAIN',1,'behav',TAB_DISC(i)%behav,'avrd=',TAB_DISC(i)%radius,'gyrd=',TAB_DISC(i)%radius/sqrt(2.D0)
  write(1004,'(A6)') '$nodty'
  write(1004,'(1X,A5,2X,I5,2X,5X,2X,5X,3(2X,A5,D14.7))') &
       'NO3xx',1,'coo1=',TAB_DISC(i)%center(1),'coo2=',TAB_DISC(i)%center(2),'coo3=',0.D0
  write(1004,'(A6)') '$tacty'
  write(1004,'(1X,A5,2X,I5,2X,A5,2X,A5,2X,A10,I5,6X,A5,D14.7)') &
       'POLYG',1,'color','BLEUx','nb_vertex=',TAB_DISC(i)%nb_vertex,'byrd=',TAB_DISC(i)%radius
!  do j=1,TAB_DISC(i)%nb_vertex
!    write(1004,'(29X,A5,D14.7,2X,A5,D14.7)') &
!        'coo1=',TAB_DISC(i)%vertex(1,j)-TAB_DISC(i)%center(1), &
!         'coo2=',TAB_DISC(i)%vertex(2,j)-TAB_DISC(i)%center(2)
!  enddo
  write(1004,'(A6)')'$$$$$$'
  write(1004,'(A6)')'      '
enddo
...
nb_wallh = 0
do i=1,n_particles
  if (TAB_DISC(i)%behav /= 'wallh')  cycle
  ...
  write(1004,'(A6)') '$tacty'
  write(1004,'(1X,A5,2X,I5,2X,A5,2X,A5,2X,A10,I5,6X,A5,D14.7)') &
       'POLYG',1,'color','REDxx','nb_vertex=',TAB_DISC(i)%nb_vertex,'byrd=',TAB_DISC(i)%radius
!  do j=1,TAB_DISC(i)%nb_vertex
!    write(1004,'(29X,A5,D14.7,2X,A5,D14.7)') &
!         'coo1=',TAB_DISC(i)%vertex(1,j)-TAB_DISC(i)%center(1), &
!         'coo2=',TAB_DISC(i)%vertex(2,j)-TAB_DISC(i)%center(2)
...
nb_wallb = 0
!do i=1,n_particles
!  if (TAB_DISC(i)%behav /= 'wallb')  cycle
!  
!  cpt = cpt + 1
!  nb_wallb = nb_wallb + 1
!  write(1004,'(A6)') '$bdyty'
...
!  write(1004,'(A6)') '$nodty'
!  write(1004,'(1X,A5,2X,I5,2X,5X,2X,5X,3(2X,A5,D14.7))') &
!       'NO3xx',1,'coo1=',TAB_DISC(i)%center(1),'coo2=',TAB_DISC(i)%center(2),'coo3=',TAB_DISC(i)%center(3)
!  write(1004,'(A6)') '$tacty'
!  write(1004,'(1X,A5,2X,I5,2X,A5,2X,A5,2X,A10,I5,6X,A5,D14.7)') &
!       'POLYG',1,'color','REDxx','nb_vertex=',TAB_DISC(i)%nb_vertex,'byrd=',TAB_DISC(i)%radius
!  do j=1,TAB_DISC(i)%nb_vertex
...
do i=1,n_particles
  ...
  nb_wallb = nb_wallb + 1
  write(1004,'(1X,A5,2X,I5,2X,A5,2X,A5,2X,A10,I5,6X,A5,D14.7)') &
       'POLYG',1,'color','REDxx','nb_vertex=',TAB_DISC(i)%nb_vertex,'byrd=',0.D0
!  do j=1,TAB_DISC(i)%nb_vertex
!    write(1004,'(29X,A5,D14.7,2X,A5,D14.7)') &
!         'coo1=',TAB_DISC(i)%vertex(1,j)-0.4750000D+02, &
!         'coo2=',TAB_DISC(i)%vertex(2,j)-3*dmin/2
!  enddo
enddo

...
real(kind=8)                                     ::  time, x_period, y_period
...
integer                                          ::  c_avg_velocity=0,&
                                                     ...
                                                     c_list_n_forces=0, c_list_t_forces=0, c_list_branches=0, & 
                                                     c_list_mobilization=0, c_fn_class=0, c_mobilization_class=0, & 
                                                     c_ctc_dir=0, c_gap_stats=0, c_qoverp_ctype=0, c_prop_nctc=0, &
                                                     c_brc_dir=0, c_frc_dir=0, c_avf_ctype=0, c_mobilization = 0, &
                                                     c_coordination_dr = 0, c_list_ctc=0

! Variables David C.
logical                                          ::  copy_input=.false.
logical                                          ::  first_over_all = .true.
integer                                          ::  qoverp_option
integer, allocatable, dimension(:)               ::  valid_part
real(kind=8)                                     ::  total_p_volume=0, sep_wall=0
...
if (command=='WALLS POSITION               :') then
  ...
  if (command=='GAP STATISTICS               :') then
    c_gap_stats = 1
    ! Uses port 123
    open (unit=123,file='./POSTPRO/GAP_STATS.DAT',status='replace')
    cycle
  end if

  if (command=='QoverP CTC TYPE              :') then
    c_qoverp_ctype = 1
    ! Uses port 124
    open (unit=124,file='./POSTPRO/QoverP_CType.DAT',status='replace')
    cycle
  end if

  if (command=='PROP NCTC                    :') then
    c_prop_nctc = 1
    ! Uses port 125
    open (unit=125,file='./POSTPRO/PROP_NCTC.DAT',status='replace')
    cycle
  end if
  ...
  if (command=='MOBILIZATION                 :') then
    c_mobilization=1
    ! Uses port 127
    open (unit=127,file='./POSTPRO/MOBILIZATION.DAT',status='replace')
    cycle
  end if

  if (command=='AVG FORCE BY CTYPE           :') then
    c_avf_ctype=1
    ! Uses port 128
    open (unit=128,file='./POSTPRO/AVG_F_CTYPE.DAT',status='replace')
    cycle
  end if

  if (command=='COORDINATION DR              :') then
    c_coordination_dr = 1
    ! Uses port 129
    cycle
  end if

  if (command=='LIST COORDINATION            :') then
    c_list_ctc = 1
    ! Uses port 130
    cycle
  end if
...
do
  ...
  if (c_gap_stats                           == 1)  call gap_stats
  if (c_qoverp_ctype                        == 1)  call qoverp_ctype
  ...
  if (c_avf_ctype                           == 1)  call avf_ctype
  if (c_coordination_dr                     == 1)  call coordination_dr
  if (c_list_ctc                            == 1)  call list_ctc
...
subroutine read_Vloc_dof_bodies
  ...
  integer                              ::  i,j,n_prpr,n_prpl,num_part,err,cd,an,n_vertices,n_faces
  ...
  if (first_over_all) then
    ...
    ! Valid 1, Invalid particle 0
    valid_part(:) = 1
  end if

  ! Checking valid particles
  ! In terms of Z
  do i=1, n_particles
    if (TAB_POLY(i)%nctc .lt. 1) valid_part(i) = 0
  end do

  ! In terms of blocked DOF
  do i=1, nb_ligneCONTACT_POLYR
...
subroutine nb_coordination
  ...
  real*8                                   :: z, zc, zp, zc1, zc11, zc2, zc3
  integer, dimension(:), allocatable       :: valid_part

  ! Looking for the valid particles
  if (allocated(valid_part)) deallocate(valid_part)
  allocate(valid_part(n_particles))
  ...
  do i=1, nb_ligneCONTACT_POLYR
    ...
    if (TAB_POLY(cd)%nctc .lt. 2) then
      valid_part(cd) = 0
    end if

    if (TAB_POLY(an)%nctc .lt. 2) then
      valid_part(an) = 0
    end if
  end do
  ...
  ! Computing the number of contacts including type
  do i=1, nb_ligneCONTACT_POLYR
    ...
    if (TAB_CONTACT_POLYR(i)%rn .lt. 1e-9) cycle
    zc = zc + 1
    ! Type of contacts
    ! One-One contacts
    ! 1 face-vertex, 11 edge-edge
    if (TAB_CONTACT_POLYR(i)%type == 1) then
      zc1 = zc1 + 1
    else if (TAB_CONTACT_POLYR(i)%type == 11) then
      zc11 = zc11 + 1
    ! Multiple
  ...
  ! Computing the number of valid particles
  do i=1, n_particles
  ...
  if (first_over_all) then
    write(100,*) '#   time     ', '      z      ', '      z1     ', '     z11     ', &
                                  '      z2     ', '      z3     ', '   n_float   '
  end if

  write(100,'(7(1X,E12.5))') time, z, zc1, zc11, zc2, zc3, real(float_part)
...
subroutine qoverp
  ...
  real*8                                   :: S1,S2,S3
  real*8                                   :: Rtik,Rnik,Rsik,q,p, Rayon_max
  real*8                                   :: max_d_vert_cd, max_d_vert_an
  ...
  do i=1,nb_ligneCONTACT_POLYR
    ...
    if (Rnik .lt. 1e-9) cycle

    if (TAB_POLY(cd)%center(3) .lt. TAB_PLAN(1)%center(3)+sep_wall*0.01) cycle
    if (TAB_POLY(cd)%center(3) .gt. TAB_PLAN(2)%center(3)-sep_wall*0.01) cycle

    if (TAB_POLY(an)%center(3) .lt. TAB_PLAN(1)%center(3)+sep_wall*0.01) cycle
    if (TAB_POLY(an)%center(3) .gt. TAB_PLAN(2)%center(3)-sep_wall*0.01) cycle
    ...
    ! The candidate
    do j=1, TAB_POLY(cd)%nb_vertex
      max_d_vert_cd = max(max_d_vert_cd, &
                      sqrt((TAB_POLY(cd)%vertex(1,j) - TAB_POLY(cd)%center(1))**2 + &
                      (TAB_POLY(cd)%vertex(2,j) - TAB_POLY(cd)%center(2))**2  + &
                      (TAB_POLY(cd)%vertex(3,j) - TAB_POLY(cd)%center(3))**2))
    end do

    ! The antagonist
    do j=1, TAB_POLY(an)%nb_vertex
      max_d_vert_an = max(max_d_vert_an, &
                      sqrt((TAB_POLY(an)%vertex(1,j) - TAB_POLY(an)%center(1))**2 + &
                      (TAB_POLY(an)%vertex(2,j) - TAB_POLY(an)%center(2))**2  + &
                      (TAB_POLY(an)%vertex(3,j) - TAB_POLY(an)%center(3))**2))
    end do

    ! Correction of the branch components if necessary
    ! X component
    if (Lik(1) .gt. (max_d_vert_an + max_d_vert_cd)) then
  ...
  if (qoverp_option == 1) then
    ...
    write(152,'(10(1X,E12.5))') time, Moment(1,1), Moment(1,2), Moment(1,3), &
...
!================================================
! Computation of q/p by contact type
!================================================
subroutine qoverp_ctype

  implicit none

  integer                                  :: i, j, cd, an
  integer                                  :: ierror_vf, ierror_ff, ierror_ee, ierror_ef, matz,lda
  integer                                  :: ierror_tot
  real*8                                   :: S1,S2,S3
  real*8                                   :: Rtik,Rnik,Rsik,Rayon_max
  real*8                                   :: q_vf,p_vf,q_ff,p_ff,q_ee,p_ee,q_ef,p_ef
  real*8                                   :: q_tot,p_tot
  real*8                                   :: max_d_vert_cd, max_d_vert_an
  real*8,dimension(3)                      :: nik,tik,sik,Lik,Fik
  real*8,dimension(3)                      :: wr_vf,wi_vf,wr_ff,wi_ff,wr_ee,wi_ee,wr_ef,wi_ef
  real*8,dimension(3)                      :: wr_tot,wi_tot
  real*8,dimension(3,3)                    :: Moment_vf, Moment_ff, Moment_ee, Moment_ef,  M
  real*8,dimension(3,3)                    :: Moment_tot
  real*8,dimension(3,3)                    :: localframe_vf,localframe_ff,localframe_ee,localframe_ef
  real*8,dimension(3,3)                    :: localframe_tot

  ! e-f : Edge - Face  - Type=2
  ! f-f: Face - Face   - Type=3 or 4
  ! e-e: Edge - Edge - Type=11
  ! v-f: vertex - face - Type=1
  ...
  do i=1,nb_ligneCONTACT_POLYR
    ...
    Moment_tot(1,1:3) = Fik(1)*Lik(1:3) + Moment_tot(1,1:3)
    Moment_tot(2,1:3) = Fik(2)*Lik(1:3) + Moment_tot(2,1:3)
    Moment_tot(3,1:3) = Fik(3)*Lik(1:3) + Moment_tot(3,1:3)

    if (TAB_CONTACT_POLYR(i)%type==1) then
      Moment_vf(1,1:3) = Fik(1)*Lik(1:3) + Moment_vf(1,1:3)
      Moment_vf(2,1:3) = Fik(2)*Lik(1:3) + Moment_vf(2,1:3)
      Moment_vf(3,1:3) = Fik(3)*Lik(1:3) + Moment_vf(3,1:3)
    else if (TAB_CONTACT_POLYR(i)%type==11) then
      Moment_ee(1,1:3) = Fik(1)*Lik(1:3) + Moment_ee(1,1:3)
      Moment_ee(2,1:3) = Fik(2)*Lik(1:3) + Moment_ee(2,1:3)
      Moment_ee(3,1:3) = Fik(3)*Lik(1:3) + Moment_ee(3,1:3)
    else if (TAB_CONTACT_POLYR(i)%type==2) then
      Moment_ef(1,1:3) = Fik(1)*Lik(1:3) + Moment_ef(1,1:3)
      Moment_ef(2,1:3) = Fik(2)*Lik(1:3) + Moment_ef(2,1:3)
      Moment_ef(3,1:3) = Fik(3)*Lik(1:3) + Moment_ef(3,1:3)
    else if (TAB_CONTACT_POLYR(i)%type==3 .or. TAB_CONTACT_POLYR(i)%type==4) then
      Moment_ff(1,1:3) = Fik(1)*Lik(1:3) + Moment_ff(1,1:3)
      Moment_ff(2,1:3) = Fik(2)*Lik(1:3) + Moment_ff(2,1:3)
      Moment_ff(3,1:3) = Fik(3)*Lik(1:3) + Moment_ff(3,1:3)
    else
      print*, 'what?? QP c type'
      stop
    end if
  end do

  Moment_vf = Moment_vf / (height*large*width)
  Moment_ee = Moment_ee / (height*large*width)
  Moment_ef = Moment_ef / (height*large*width)
  Moment_ff = Moment_ff / (height*large*width)

  Moment_tot = Moment_tot / (height*large*width)

  ! Computing the eigenvalues for each tensor
  ...
  call rg (lda, 3, M, wr_tot, wi_tot, matz, localframe_tot, ierror_tot)

  if ( wr_tot(1)==wr_tot(2) .and. (wr_tot(2)==wr_tot(3)) ) then
    S1=wr_tot(1)
    S2=S1
    S3=S1
  else
    S3 = max( wr_tot(1),max(wr_tot(2),wr_tot(3)))
    S1 = min( wr_tot(1),min(wr_tot(2),wr_tot(3)))
    if (wr_tot(1)==wr_tot(2)) then
      if (S1==wr_tot(1)) S2=S1
      if (S3==wr_tot(1)) S2=S3
    else if (wr_tot(3)==wr_tot(2)) then
      if (S1==wr_tot(2)) S2=S1
      if (S3==wr_tot(3)) S2=S3
    else
      do i=1,3
        if ((wr_tot(i)<S3) .and. (wr_tot(i)>S1)) S2=wr_tot(i)
      end do
    end if
  end if
  ...
  call rg (lda, 3, M, wr_vf, wi_vf, matz, localframe_vf, ierror_vf)

  if ( wr_vf(1)==wr_vf(2) .and. (wr_vf(2)==wr_vf(3)) ) then
    S1=wr_vf(1)
    S2=S1
    S3=S1
  else
    S3 = max( wr_vf(1),max(wr_vf(2),wr_vf(3)))
    S1 = min( wr_vf(1),min(wr_vf(2),wr_vf(3)))
    if (wr_vf(1)==wr_vf(2)) then
      if (S1==wr_vf(1)) S2=S1
      if (S3==wr_vf(1)) S2=S3
    else if (wr_vf(3)==wr_vf(2)) then
      if (S1==wr_vf(2)) S2=S1
      if (S3==wr_vf(3)) S2=S3
    else
      do i=1,3
        if ((wr_vf(i)<S3) .and. (wr_vf(i)>S1)) S2=wr_vf(i)
      end do
    end if
  end if
  ...
  call rg (lda, 3, M, wr_ee, wi_ee, matz, localframe_ee, ierror_ee)

  if ( wr_ee(1)==wr_ee(2) .and. (wr_ee(2)==wr_ee(3)) ) then
    S1=wr_ee(1)
    S2=S1
    S3=S1
  else
    S3 = max( wr_ee(1),max(wr_ee(2),wr_ee(3)))
    S1 = min( wr_ee(1),min(wr_ee(2),wr_ee(3)))
    if (wr_ee(1)==wr_ee(2)) then
      if (S1==wr_ee(1)) S2=S1
      if (S3==wr_ee(1)) S2=S3
    else if (wr_ee(3)==wr_ee(2)) then
      if (S1==wr_ee(2)) S2=S1
      if (S3==wr_ee(3)) S2=S3
    else
      do i=1,3
        if ((wr_ee(i)<S3) .and. (wr_ee(i)>S1)) S2=wr_ee(i)
      end do
    end if
  end if
  ...
  call rg (lda, 3, M, wr_ef, wi_ef, matz, localframe_ef, ierror_ef)

  if ( wr_ef(1)==wr_ef(2) .and. (wr_ef(2)==wr_ef(3)) ) then
    S1=wr_ef(1)
    S2=S1
    S3=S1
  else
    S3 = max( wr_ef(1),max(wr_ef(2),wr_ef(3)))
    S1 = min( wr_ef(1),min(wr_ef(2),wr_ef(3)))
    if (wr_ef(1)==wr_ef(2)) then
      if (S1==wr_ef(1)) S2=S1
      if (S3==wr_ef(1)) S2=S3
    else if (wr_ef(3)==wr_ef(2)) then
      if (S1==wr_ef(2)) S2=S1
      if (S3==wr_ef(3)) S2=S3
    else
      do i=1,3
        if ((wr_ef(i)<S3) .and. (wr_ef(i)>S1)) S2=wr_ef(i)
      end do
    end if
  end if
  ...
  call rg (lda, 3, M, wr_ff, wi_ff, matz, localframe_ff, ierror_ff)

  if ( wr_ff(1)==wr_ff(2) .and. (wr_ff(2)==wr_ff(3)) ) then
    S1=wr_ff(1)
    S2=S1
    S3=S1
  else
    S3 = max( wr_ff(1),max(wr_ff(2),wr_ff(3)))
    S1 = min( wr_ff(1),min(wr_ff(2),wr_ff(3)))
    if (wr_ff(1)==wr_ff(2)) then
      if (S1==wr_ff(1)) S2=S1
      if (S3==wr_ff(1)) S2=S3
    else if (wr_ff(3)==wr_ff(2)) then
      if (S1==wr_ff(2)) S2=S1
      if (S3==wr_ff(3)) S2=S3
    else
      do i=1,3
        if ((wr_ff(i)<S3) .and. (wr_ff(i)>S1)) S2=wr_ff(i)
      end do
    end if
  end if
  ...
  if (first_over_all) then
    write(122,*) '#   time     ', '     q/p_vf   ','     q/p_ee   ','   q/p_ef   ', '   q/p_ff    ', '   q/p_tot   '
  end if

  write(122,'(6(1X,E12.5))') time, q_vf/p_tot, q_ee/p_tot, q_ef/p_tot, q_ff/p_tot, q_tot/p_tot

  print*, 'Writing q/p by c type ---> Ok!'

end subroutine qoverp_ctype
...
subroutine anisotropy_branch
  ...
  real*8, dimension(3)                     :: wrn, win, wrf, wif
  real*8, dimension(3)                     :: Lik, max_d_vert_cd, max_d_vert_an
  real*8, dimension(3,3)                   :: Fabric_N,Fabric_T, Fabric_F
  real*8, dimension(3,3)                   :: localframeN, localframeF
  ...
  do i=1,nb_ligneCONTACT_POLYR
    ...
    ! Building the branch in the normal direction
    ...
    ! Building the branch in tangential direction
  ...
  ! Normalizing by the average branch 
  Fabric_N = Fabric_N / (cpt*av_length)
  Fabric_T = Fabric_T / (cpt*av_length)
  
  ! Building the full branch tensor N+T (Build this always before using rg!!!)
  Fabric_F = Fabric_N + Fabric_T
  ...
  ! Finding the eigen values of the full tensor
  call rg(lda, 3, Fabric_F, wrf, wif, matz, localframeF, ierror)
...
subroutine prop_nctc
  ...
  do i=1, n_particles
    if (TAB_POLY(i)%nctc == 0 ) then
      l_p0 = l_p0 + 1.
      float_par = float_par + 1
    else if (TAB_POLY(i)%nctc == 1) then
      l_p1 = l_p1 + 1.
      float_par = float_par + 1
    else if (TAB_POLY(i)%nctc == 2) then
      l_p2 = l_p2 + 1.
    else if (TAB_POLY(i)%nctc == 3) then
      l_p3 = l_p3 + 1.
    else if (TAB_POLY(i)%nctc == 4) then
      l_p4 = l_p4 + 1.
    else if (TAB_POLY(i)%nctc == 5) then
      l_p5 = l_p5 + 1.
    else if (TAB_POLY(i)%nctc == 6) then
      l_p6 = l_p6 + 1.
    else if (TAB_POLY(i)%nctc == 7) then
      l_p7 = l_p7 + 1.
    else if (TAB_POLY(i)%nctc == 8) then
      l_p8 = l_p8 + 1.
    else if (TAB_POLY(i)%nctc == 9) then
      l_p9 = l_p9 + 1.
    else if (TAB_POLY(i)%nctc == 10) then
      l_p10 = l_p10 + 1.
    else if (TAB_POLY(i)%nctc == 11) then
      l_p11 = l_p11 + 1.
    else if (TAB_POLY(i)%nctc == 12) then
      l_p12 = l_p12 + 1.
    else if (TAB_POLY(i)%nctc == 13) then
      l_p13 = l_p13 + 1.
    else if (TAB_POLY(i)%nctc == 14) then
      l_p14 = l_p14 + 1.
    else if (TAB_POLY(i)%nctc == 15) then
      l_p15 = l_p15 + 1.
    else if (TAB_POLY(i)%nctc == 16) then
      l_p16 = l_p16 + 1.
    else if (TAB_POLY(i)%nctc == 17) then
      l_p17 = l_p17 + 1.
    else if (TAB_POLY(i)%nctc == 18) then
      l_p18 = l_p18 + 1.
    else if (TAB_POLY(i)%nctc == 19) then
      l_p19 = l_p19 + 1.
    end if
  end do

  l_p0  = l_p0 /real(n_particles - float_par)
  l_p1  = l_p1 /real(n_particles - float_par)
  l_p2  = l_p2 /real(n_particles - float_par)
  l_p3  = l_p3 /real(n_particles - float_par)
  l_p4  = l_p4 /real(n_particles - float_par)
  l_p5  = l_p5 /real(n_particles - float_par)
  l_p6  = l_p6 /real(n_particles - float_par)
  l_p7  = l_p7 /real(n_particles - float_par)
  l_p8  = l_p8 /real(n_particles - float_par)
  l_p9  = l_p9 /real(n_particles - float_par)
  l_p10 = l_p10/real(n_particles - float_par)
  l_p11 = l_p11/real(n_particles - float_par)
  l_p12 = l_p12/real(n_particles - float_par)
  l_p13 = l_p13/real(n_particles - float_par)
  l_p14 = l_p14/real(n_particles - float_par)
  l_p15 = l_p15/real(n_particles - float_par)
  l_p16 = l_p16/real(n_particles - float_par)
  l_p17 = l_p17/real(n_particles - float_par)
  l_p18 = l_p18/real(n_particles - float_par)
  l_p19 = l_p19/real(n_particles - float_par)

  if (first_over_all) then
    ! Writing
    write(125,*) '#   Time    ', '     p0    ','     p1    ','     p2    ', &
                                 '     p3    ','     p4    ','     p5    ', &
                                 '     p6    ','     p7    ','     p8    ', &
                                 '     p9    ','    p10    ','     11    ', &
                                 '    p12    ','    p13    ','    p14    ', &
  ...
  write(125,'(21(1X,E12.5))') time, l_p0, l_p1, l_p2, l_p3, l_p4, l_p5, l_p6, l_p7, &
                                    l_p8, l_p9, l_p10, l_p11, l_p12, l_p13, l_p14, & 
                                    l_p15, l_p16, l_p17, l_p18, l_p19
...
!==============================================================================
! Computing the coordination by class and by particle relative size DR
!==============================================================================
subroutine coordination_dr
  
  implicit none
  
  integer                                  :: i, j, cd, an, n_bin, c_ctype, n_valid_par
  integer                                  :: cur_bin
  real*8                                   :: r_max, r_min, interval
  real*8, dimension(:,:), allocatable      :: bins
  character(len=24)                        :: coor_c_file
  ...
  ! Opening the file
  open(unit=129,file=coor_c_file,status='replace')
  ...
  ! Computing the number of valid particles
  do i=1, n_particles
    if (valid_part(i)==1) n_valid_par = n_valid_par + 1
  end do
  ...
  ! Adding contacts after size
  do i=1, n_particles
    if (valid_part(i) .gt. 0) then 
      c_ctype = 0
      cur_bin = int(n_bin*(TAB_POLY(i)%Rmax - r_min)/(r_max-r_min))+1
      do j=1, nb_ligneCONTACT_POLYR
        if  (TAB_CONTACT_POLYR(j)%nature /= 'PRPRx') cycle
        if (TAB_CONTACT_POLYR(j)%rn .lt. 1.D-9) cycle

        cd   = TAB_CONTACT_POLYR(j)%icdent
        an   = TAB_CONTACT_POLYR(j)%ianent

        if (cd == i .or. an == i) then
          if (TAB_CONTACT_POLYR(j)%type == 1) then
            c_ctype = 1
          else if (TAB_CONTACT_POLYR(j)%type == 11) then
            c_ctype = 2
          else if (TAB_CONTACT_POLYR(j)%type == 2) then
            c_ctype = 3
          else if (TAB_CONTACT_POLYR(j)%type == 3 .or. TAB_CONTACT_POLYR(j)%type == 4) then
            c_ctype = 4
          else
            print*, 'THe heck!'
            print*, TAB_CONTACT_POLYR(j)%type
            print*, c_ctype
          end if
          bins(cur_bin,c_ctype+1) = bins(cur_bin,c_ctype+1) + 1
        end if
      end do
      bins(cur_bin,6) = bins(cur_bin,6) + 1
    end if
  end do

  ! Computing the corresponding coordination
  bins(:,2) = bins(:,2)/bins(:,6)
  bins(:,3) = bins(:,3)/bins(:,6)
  bins(:,4) = bins(:,4)/bins(:,6)
  bins(:,5) = bins(:,5)/bins(:,6)

  ! Writing
  write(129,*) '#   Class    ', '    z_vf     ', '    z_ee     ', '    z_ef     ', &
               '    z_ff     ', '   tot_nctc  ', 'valid:', n_valid_par

  !print*, bins  
  
  do i=1, n_bin
    if (bins(i,6) .gt. 0.) then 
      write(129,'(6(1X,E12.5))') bins(i,1), bins(i,2), bins(i,3), bins(i,4), bins(i,5), bins(i,6)
    end if 
  end do 

  close(129)

  print*, 'Coordination DR    ---> Ok!'
...
subroutine draw
  ...
  write(114, '(2(I6,A))') n_l_faces + n_walls*6, ' ' , (n_l_faces*4)+(n_walls*6*5)
  ...
  write(114, '(2(I6,A))') n_l_faces + n_walls*6
  ...
  do i=1, nb_ligneCONTACT_POLYR
    ...
    ! Far from the walls
    if (TAB_POLY(l_cdt)%center(3) .lt. TAB_PLAN(1)%center(3) + 0.01*disp_wall_times) cycle 
    if (TAB_POLY(l_cdt)%center(3) .gt. TAB_PLAN(2)%center(3) - 0.01*disp_wall_times) cycle 

    if (TAB_POLY(l_ant)%center(3) .lt. TAB_PLAN(1)%center(3) + 0.01*disp_wall_times) cycle 
    if (TAB_POLY(l_ant)%center(3) .gt. TAB_PLAN(2)%center(3) - 0.01*disp_wall_times) cycle 
  ...
  ! How many fields there will be?
  n_l_fields = 4                          ! They are: Type RN, RT, Mobilization, ctc_type
                                          ! ...
  ...
  ! Naming the field, the dimension of the data, and number of lines (as before FIELD), and data type
  write(114,'(A,I1,I8,A)') 'Ctc_type ', 1, (f_counter+f_counter_periodic*2)*6, ' float'
  k = 0
  do i=1, nb_ligneCONTACT_POLYR
    l_cdt = TAB_CONTACT_POLYR(i)%icdent
    l_ant = TAB_CONTACT_POLYR(i)%ianent
    ...
    if (sqrt(Lik(1)**2 + Lik(2)**2 + Lik(3)**2) .gt. (TAB_POLY(l_cdt)%Rmax + TAB_POLY(l_ant)%Rmax)) then 
      do j=1, 12
        k=k+1
        if(k .le. 9) then
          write(114, '(F15.7)', advance='no') real(TAB_CONTACT_POLYR(i)%type)
        else 
          write(114, '(F15.7)') real(TAB_CONTACT_POLYR(i)%type)
          k=0
        end if 
      end do
    else
...
subroutine list_n_forces
  ...
  do i=1,nb_ligneCONTACT_POLYR
    ...
    ! Active contacts
    if (TAB_CONTACT_POLYR(i)%rn .lt. 1.D-9) cycle
...
subroutine list_t_forces
  ...
  ! Opening the file
  open(unit=126,file=l_file,status='replace')
  ...
  do i=1,nb_ligneCONTACT_POLYR
    ...
    write(126, '(E15.7)') (TAB_CONTACT_POLYR(i)%rt**2+TAB_CONTACT_POLYR(i)%rs**2)**0.5
  end do
...
subroutine list_branches
  ...
  do i=1,nb_ligneCONTACT_POLYR
    ...
    ! Maximal possible branch for these two particles
    d_vert = sqrt(max_d_vert_cd(1)**2 + max_d_vert_cd(2)**2 + max_d_vert_cd(3)**2) + &
             sqrt(max_d_vert_an(1)**2 + max_d_vert_an(2)**2 + max_d_vert_an(3)**2)

    ! Not including contacts in the periodic zones
    if (sqrt(Lik(1)**2 + Lik(2)**2 + Lik(3)**2) .gt. d_vert) cycle
...
!==============================================================================
! List of number of contacts per particle
!==============================================================================
subroutine list_ctc
  ...
  if (compteur_clout<10) then
    ...
    print*, "Not enough file names :: List of contact number"
    stop
  end if
  ...
  do i=1,n_particles
    ! Contacts between particles
    if (TAB_POLY(i)%nctc .lt. 2) cycle

    write(130, '(E15.7)') TAB_POLY(i)%nctc
  end do

  close(130)

  print*, 'Write List NC     ---> Ok!'

end subroutine list_ctc
...
subroutine mobilization
  ...
  do i=1, nb_ligneCONTACT_POLYR
    ...
    ! CAREFUL!! Writing explicitly the coefficient of friction!!!!
    mob_ave = mob_ave + sqrt(TAB_CONTACT_POLYR(i)%rs**2 + TAB_CONTACT_POLYR(i)%rt**2)/(0.4*TAB_CONTACT_POLYR(i)%rn)
    act_ctc = act_ctc + 1
  end do
  ...
  if (first_over_all) then
    ! Writing
    write(127,*) '#   Time    ', ' <|ft|/mu fn>   '
  end if

  write(127,'(2(1X,E15.7))') time, mob_ave
...
subroutine ctc_dir
  ...
  ! Normalizing
  theta_interval(:,3) = theta_interval(:,2)/(contotal)!*interval)
...
subroutine branch_dir
  ...
  do i=1, ninter
    ...
    do j=1,nb_ligneCONTACT_POLYR
      ...
      ! Not joining contacts in the periodic zones
      if (sqrt(Lik(1)**2 + Lik(2)**2 + Lik(3)**2) .gt. 3.*(TAB_POLY(cd)%Rmax + TAB_POLY(an)%Rmax)) cycle
      ...
      pro_t = Lik(1)*tanik(1) + Lik(2)*tanik(2) + Lik(3)*tanik(3)
...
! Gap statistics
!==============================================================================
subroutine gap_stats

  implicit none

  integer                                   :: i
  real*8                                    :: g_mean, g_max, g_min, nc_count

  ! Counting how many contacts
  nc_count = 0.0
  g_mean   = 0.0
  g_max    = -1.0

  ! Considering only negative gaps... ALL ORIGINAL CONTACTS
  do i=1, nb_ligneCONTACT
    if (TAB_CONTACT(i)%gapTT .gt. 0) cycle
    nc_count = nc_count + 1

    g_mean = g_mean + abs(TAB_CONTACT(i)%gapTT)
    g_max = max(g_max,abs(TAB_CONTACT(i)%gapTT))
  end do

  g_mean = g_mean/nc_count

  if (first_over_all) then
    write(121,*) '#   time     ', '   Nc_count  ', '  g_mean  ', '  g_max  '
  end if

  write(121,'(4(1X,E12.5))') time, nc_count, g_mean, g_max

  print*, 'Gap statistics        ---> Ok!'

end subroutine gap_stats

!==============================================================================
! Computing average forces by contact type
!==============================================================================
subroutine avf_ctype

  implicit none

  integer                                   :: i, count1, count11, count2, count3, count_tot
  real*8                                    :: av_ct1, av_ct11, av_ct2, av_ct3, avg_force
  ...
  do i=1, nb_ligneCONTACT_POLYR
    ...
    if (TAB_CONTACT_POLYR(i)%nature /= 'PRPRx') cycle
    !Only active contacts
    if (TAB_CONTACT_POLYR(i)%rn .le. 1e-9) cycle

    if (TAB_CONTACT_POLYR(i)%type == 1) then 
      av_ct1 = av_ct1 + TAB_CONTACT_POLYR(i)%rn
      count1 = count1 + 1
    else if (TAB_CONTACT_POLYR(i)%type == 11) then 
      av_ct11 = av_ct11 + TAB_CONTACT_POLYR(i)%rn
      count11 = count11 + 1
    else if (TAB_CONTACT_POLYR(i)%type == 2) then 
      av_ct2 = av_ct2 + TAB_CONTACT_POLYR(i)%rn
      count2 = count2 + 1
    else if (TAB_CONTACT_POLYR(i)%type == 3 .or. TAB_CONTACT_POLYR(i)%type==4) then
      av_ct3 = av_ct3 + TAB_CONTACT_POLYR(i)%rn
      count3 = count3 + 1
    else
      print*, "The heck!!! AVG_F_CTYPE"
      stop
    end if
    avg_force = avg_force + TAB_CONTACT_POLYR(i)%rn
    count_tot = count_tot + 1
  end do

  av_ct1  = av_ct1 /real(count1)
  av_ct11 = av_ct11/real(count11)
  av_ct2  = av_ct2 /real(count2)
  av_ct3  = av_ct3 /real(count3)

  avg_force = avg_force/real(count_tot)

  if (first_over_all) then
    write(128,*) '#   time     ', '   av_c1   ', '   av_c11  ', '   av_c2   ', '   av_c3   ', '   av_fn   '
  end if

  write(128,'(6(1X,E15.7))') time, av_ct1, av_ct11, av_ct2, av_ct3, avg_force

  print*, 'Average force c type  ---> Ok!'

end subroutine avf_ctype
...
subroutine close_all
  ...
  if (c_gap_stats                           == 1) close (121)
  if (c_qoverp_ctype                        == 1) close (122)
  if (c_prop_nctc                           == 1) close (125)
  if (c_mobilization                        == 1) close (127)
  if (c_avf_ctype                           == 1) close (128)

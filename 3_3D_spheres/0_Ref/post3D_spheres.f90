...
integer                                         ::  type_box=0,calcul_vitesse_moyenne=0,calcul_coordination=0,&
                                                    calcul_qoverp=0,calcul_compacity=0,calcul_contact_anisotropies=0,&
                                                    calcul_pdf=0,calcul_ksi_reseau=0,&
...
do 
  ...
  if (calcul_contact_anisotropies           == 1)  call contact_anisotropies
...
subroutine nb_coordination
  ...
  real(kind=8)                                  :: z,zc,zp
  real(kind=8)                                  :: P0,P1,P2,P3,P4,P5,P6,P7,P8,P9,P10,P11,P12,zc3,float_part,nb_part_without_paroi,&
                                                   z_float,dist,&
                                                   P1_all,P2_all,P3_all,P4_all,P5_all,P6_all,P7_all,P8_all,P9_all,P10_all,P11_all,& 
                                                   P12_all
  integer                                       :: i,j,cd,an
...
!  if (TAB_CONTACT(i)%rn==0.D0) cycle
  ...
  write(100,'(40(1X,D12.5))') time,Def,Def_H,z,P0,P1,P2,P3,P4,P5,P6,P7,P8,P9,&
...
subroutine compacity
  ...
  do i=1,n_particles
    Volume_grains  = Volume_grains  + TAB_SPHERE(i)%Volume
  end do
...
if (type_box==0)&
write(104,'(31(1X,D12.5))') time,Def,Def_H,&
...
if (type_box==0)&
write(1005,'(31(1X,D12.5))') time,Def,Def_H,&
...
if (type_box==0)&
write(105,'(31(1X,D12.5))') time,Def,Def_H,Sa,Sa_NS
...
do i=1,nb_ligneCONTACT
  ...
  an   = TAB_CONTACT(i)%ianent
!  if (TAB_CONTACT(i)%rn<0.000000000001*Mean_total_Normal_force) cycle
  if (cd>n_particles.or.an>n_particles) cycle
...
q = (S3-S1)/2
p = (S1+S2+S3)/3
write(102,'(20(1X,D12.5))') time,Def,-Moment(1,3)/ Moment(3,3),&
...
do i=1,nb_ligneCONTACT
  ...
  Lik(1:3) = TAB_CONTACT(i)%coor_ctc(1:3)-TAB_SPHERE(cd)%center(1:3)
  if ( sqrt( Lik(1)**2 + Lik(2)**2 + Lik(3)**2) > 2*( TAB_SPHERE(cd)%Rmax + TAB_SPHERE(an)%Rmax ) ) then
    Rayon_max = sqrt( ( Lik(1)**2 + Lik(2)**2 + Lik(3)**2) ) 
    lik(:) = Lik(:) / Rayon_max
  ...
  Lik(1:3) = TAB_CONTACT(i)%coor_ctc(1:3)-TAB_SPHERE(an)%center(1:3)
  if ( sqrt( Lik(1)**2 + Lik(2)**2 + Lik(3)**2) > 2*( TAB_SPHERE(cd)%Rmax + TAB_SPHERE(an)%Rmax ) ) then
    Rayon_max = sqrt( ( Lik(1)**2 + Lik(2)**2 + Lik(3)**2) ) 
    lik(:) = Lik(:) / Rayon_max
...
if (type_box==0) write(1003,'(20(1X,D12.5))') time,Def,qprime/pprime,&
                                              qprime/pprime,&
                                              q1p,&
                                              q2p,&
                                              q3p,&
...
!        dist = sqrt( (TAB_SPHERE(i)%center(1)-TAB_SPHERE(j)%center(1))**2 + &
...
!  if (TAB_SPHERE(i)%nctc > 0 ) then
    ...
    if ( (TAB_SPHERE(i)%center(2) > 0.30+0.150).and.(TAB_SPHERE(i)%center(2) < 0.30-0.150) ) cycle
    if ( (TAB_SPHERE(i)%center(1) > 0.25+0.125).and.(TAB_SPHERE(i)%center(1) < 0.25-0.125) ) cycle
    if ( (TAB_SPHERE(i)%center(3) > 0.25+0.125).and.(TAB_SPHERE(i)%center(3) < 0.25-0.125) ) cycle
...
allocate(Tab_correlation(int(P0),10000,2))
allocate(density_part(n_particles))
...
!        if ( (dist < ksi) ) then 
      ...
      print*, ksi/rmoyen, particule_i,n_particles
      ksi = ksi + epsilon
      nb_ksi = nb_ksi + 1
      if (ksi>20*rmoyen) exit

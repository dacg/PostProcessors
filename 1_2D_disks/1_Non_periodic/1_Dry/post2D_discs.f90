...
type  ::  T_CORPS
   integer                                  ::  num,lieu,nb_vertex,n_ctc
...
type(T_CORPS),allocatable,dimension(:)      ::  TAB_DISC,TAB_PLAN,TAB_DISC_INI
...
integer                                    ::  calcul_orientation=0,calcul_construct_bodies=0, &
                                               c_draw = 0, c_n_ctc_probability = 0, c_sample_state=0
...
integer                                    ::  n_out_files
logical                                    ::  copy_input=.false., file_ex
...
do
  ...
  if (command=='SAMPLE STATE                 :') then
    c_sample_state = 1
    open (unit=112,file='./POSTPRO/SAMPLE_STATE.DAT',status='replace')
    cycle
  end if
...
!==============================================================================
  do
    ...
    if(fin_post .neqv. .true.) then
      ...
      if (c_sample_state              == 1)  then
        if (first_over_all) then
          inquire(file='num_out_files.DAT', exist=file_ex)
          if(file_ex) call execute_command_line('rm num_out_files.DAT')
          call execute_command_line('files=(./OUTBOX/DOF.OUT.*) ; num_files="${#files[@]}" ; echo $num_files >> num_out_files.DAT')
          !call execute_command_line('echo $num_files >> num_out_files.DAT')
          open(unit=201,file='num_out_files.DAT') 
          read(201,*) n_out_files
          close (201)
        end if
        call sample_state
      end if
...
SUBROUTINE read_Vloc_dof_bodies
  ...
  do i=1,nb_ligneCONTACT
    ...
    if (TAB_CONTACT(i)%rn .le. 1.0E-9) cycle

    TAB_DISC(icdent)%n_ctc = TAB_DISC(icdent)%n_ctc + 1
    TAB_DISC(ianent)%n_ctc = TAB_DISC(ianent)%n_ctc + 1
  end do
...
subroutine qoverp
  ...
  write(103,'(4(1X,E12.5))') time, S1,S2,(S1-S2)/(S1+S2)
...
subroutine nb_coordination
  ...
  do i=1, nb_ligneCONTACT
    ...
    if(TAB_CONTACT(i)%nature == 'DKJCx') valid_part(cd) = 0
    if(TAB_DISC(cd)%n_ctc .lt. 2) valid_part(cd) = 0
  end do
  ...
  do i=1,nb_ligneCONTACT
    ...
    ! Active contacts
    if (TAB_CONTACT(i)%rn .gt. 1.e-9) then
      zc=zc+1
    end if 
  end do
...
subroutine walls_force
  ...
  do i=1, nb_ligneCONTACT
    if (TAB_CONTACT(i)%nature== 'DKJCx') then
      if (TAB_CONTACT(i)%rn .lt. 1e-9) cycle
...
subroutine anisotropy_contact
  ...
  write(104,'(2(1X,E12.5))') time, 2*(F1-F2)
...
!==============================================================================
! Writing the sample state for tessellation routines
!==============================================================================
subroutine sample_state
  ...
  if (first_over_all) then
     write(112,'(2(1X,I8))') n_particles, n_walls
  end if

  ! Writing the last state of the simulation
  if (compteur_clout == n_out_files) then
    do i=1, n_particles
      write(112,'(3(1X,E12.5))') TAB_DISC(i)%radius, TAB_DISC(i)%center(1), TAB_DISC(i)%center(2)
    end do

    do i=1, n_walls
      write(112,'(4(1X,E12.5))') TAB_PLAN(i)%center(1), TAB_PLAN(i)%center(2), TAB_PLAN(i)%ax1, TAB_PLAN(i)%ax2
    end do

    print*, 'Write Last Sample State       ---> Ok!'
  end if
...
subroutine orientation
  ...
  do i=1,Nsect_orientation-1
    write(1000,'(2(1X,E14.7))') sect*(i-1), sqrt( (cos(sect*(i-1))*tab_C(i))**2 + &
                                                  (sin(sect*(i-1))*tab_C(i))**2)   
    write(1000,'(2(1X,E14.7))') sect*(i)  , sqrt( (cos(sect*(i))*tab_C(i))**2   + &
                                                  (sin(sect*(i))*tab_C(i))**2)
                                                  
    write(1001,'(2(1X,E14.7))') sect*(i-1), sqrt( (cos(sect*(i-1))*tab_N(i))**2 + &
                                                  (sin(sect*(i-1))*tab_N(i))**2)   
    write(1001,'(2(1X,E14.7))') sect*(i)  , sqrt( (cos(sect*(i))*tab_N(i))**2   + &
                                                  (sin(sect*(i))*tab_N(i))**2)
                                                  
    write(1002,'(2(1X,E14.7))') sect*(i-1), sqrt( (cos(sect*(i-1))*tab_T(i))**2 + &
                                                  (sin(sect*(i-1))*tab_T(i))**2)   
    write(1002,'(2(1X,E14.7))') sect*(i)  , sqrt( (cos(sect*(i))*tab_T(i))**2   + &
                                                  (sin(sect*(i))*tab_T(i))**2)
  end do
  do i=1,Nsect_orientation-1
    write(1000,'(2(1X,E14.7))') sect*(i-1)+pi, sqrt( (cos(sect*(-Nsect_orientation/2+i-1))*tab_C(i))**2 + &
                                                     (sin(sect*(-Nsect_orientation/2+i-1))*tab_C(i))**2)
    write(1000,'(2(1X,E14.7))') sect*(i)+pi  , sqrt( (cos(sect*(-Nsect_orientation/2+i))*tab_C(i))**2   + &
                                                     (sin(sect*(-Nsect_orientation/2+i))*tab_C(i))**2)
                                                     
    write(1001,'(2(1X,E14.7))') sect*(i-1)+pi, sqrt( (cos(sect*(-Nsect_orientation/2+i-1))*tab_N(i))**2 + &
                                                     (sin(sect*(-Nsect_orientation/2+i-1))*tab_N(i))**2)
    write(1001,'(2(1X,E14.7))') sect*(i)+pi  , sqrt( (cos(sect*(-Nsect_orientation/2+i))*tab_N(i))**2   + &
                                                     (sin(sect*(-Nsect_orientation/2+i))*tab_N(i))**2)
                                                     
    write(1002,'(2(1X,E14.7))') sect*(i-1)+pi, sqrt( (cos(sect*(-Nsect_orientation/2+i-1))*tab_T(i))**2 + &
                                                     (sin(sect*(-Nsect_orientation/2+i-1))*tab_T(i))**2)
    write(1002,'(2(1X,E14.7))') sect*(i)+pi  , sqrt( (cos(sect*(-Nsect_orientation/2+i))*tab_T(i))**2   + &

    ...
    do
      ...
      if (text == '$bdyty') then
        ...
        !  Reading the walls
          ...
          ! As function of the wall type the next line are read or not. 
          if (w_type == 0) then
            ...
            ! Cluster wall! ----> Temporarily
            TAB_PLAN(i-n_particles)%ax1=3.0
            TAB_PLAN(i-n_particles)%ax2=0.1
            TAB_PLAN(i-n_particles)%center(:)=0.D0
            TAB_PLAN(i-n_particles)%behav = 'WALL_' 
            ...
            ! Vertices
            TAB_PLAN(i-n_particles)%vertex_ref(1,1) =  3.0
            TAB_PLAN(i-n_particles)%vertex_ref(2,1) =  0.1
            TAB_PLAN(i-n_particles)%vertex_ref(3,1) =  0.D0
            
            TAB_PLAN(i-n_particles)%vertex_ref(1,2) = (- 3.0)
            TAB_PLAN(i-n_particles)%vertex_ref(2,2) =  0.1
            TAB_PLAN(i-n_particles)%vertex_ref(3,2) =  0.D0
            
            TAB_PLAN(i-n_particles)%vertex_ref(1,3) = (- 3.0)
            TAB_PLAN(i-n_particles)%vertex_ref(2,3) = (- 0.1)
            TAB_PLAN(i-n_particles)%vertex_ref(3,3) =  0.D0
            
            TAB_PLAN(i-n_particles)%vertex_ref(1,4) =  3
            TAB_PLAN(i-n_particles)%vertex_ref(2,4) = (- 0.1)
            TAB_PLAN(i-n_particles)%vertex_ref(3,4) =  0.D0
  ...
  if (err/=0) then
    ...
    do 
      ...
      if (text2 == '$icdan  DKDKx') then
        ...
        if(icdent == n_particles + 1 .or. ianent == n_particles+ 1) then
           TAB_CONTACT(nc_disc)%nature='DKJCx'
        else
           TAB_CONTACT(nc_disc)%nature='DKDKx'
        end if
        
        if(icdent == n_particles + 2 .or. ianent == n_particles+ 2) then
           TAB_CONTACT(nc_disc)%nature='DKJCx'
        else
           TAB_CONTACT(nc_disc)%nature='DKDKx'
        end if
...
subroutine wallsposition
  ...
  write(108,'(5(1X,D12.5))') time, TAB_PLAN(1)%center(1), TAB_PLAN(1)%center(2) &
                                 , TAB_PLAN(2)%center(1), TAB_PLAN(2)%center(2)
...
subroutine nb_coordination
  ...
  do i=1,nb_ligneCONTACT
    ...
    if (TAB_CONTACT(i)%in_box) then
      ! Active contacts
      if (TAB_CONTACT(i)%rn .gt. 0.D0) then
        zc=zc+1
      end if 
    end if
  end do
...
subroutine walls_force
  ...
  do i=1, nb_ligneCONTACT
    if(TAB_CONTACT(i)%ianent == idw1) then
      ft1 = ft1 + (TAB_CONTACT(i)%rn*TAB_CONTACT(i)%n(1) - TAB_CONTACT(i)%rt*TAB_CONTACT(i)%n(2))
      fn1 = fn1 + (TAB_CONTACT(i)%rn*TAB_CONTACT(i)%n(2) + TAB_CONTACT(i)%rt*TAB_CONTACT(i)%n(1))
      
    elseif(TAB_CONTACT(i)%ianent == idw2) then
      ft2 = ft2 + (TAB_CONTACT(i)%rn*TAB_CONTACT(i)%n(1) - TAB_CONTACT(i)%rt*TAB_CONTACT(i)%n(2))
      fn2 = fn2 + (TAB_CONTACT(i)%rn*TAB_CONTACT(i)%n(2) + TAB_CONTACT(i)%rt*TAB_CONTACT(i)%n(1))
  ...
  write(109,'(5(1X,D12.5))') time, fn1, ft1, fn2, ft2
...
subroutine anisotropy_contact
  ...
  write(104,'(2(1X,D12.5))') time, 2*(F1-F2)
...
subroutine test_position_contact
  ...
  do i=1,n_particles
    xmax = max(xmax,TAB_DISC(i)%center(1)+TAB_DISC(i)%radius)
    ymax = max(ymax,TAB_DISC(i)%center(2)+TAB_DISC(i)%radius)
    xmin = min(xmin,TAB_DISC(i)%center(1)-TAB_DISC(i)%radius)
    ymin = min(ymin,TAB_DISC(i)%center(2)-TAB_DISC(i)%radius)
  end do

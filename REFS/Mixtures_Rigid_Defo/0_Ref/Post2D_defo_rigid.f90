...
type  ::  T_CORPS_deformable
  integer                                                 ::  num,nb_neighbor,nb_ctc
...
integer                                     ::  calcul_anisotropie_contact=0
...
!################# Over all time steps
  ...
  if (command=='COORDINATION                 :') then
    ...
    open (unit=101,file='COORDINATION.DAT',status='replace')
  cycle
  end if
  if (command=='ANISOTROPIES FROM CONTACTS   :') then
    calcul_anisotropie_contact=1
    open (unit=102,file='ANISOTROPIES_FROM_CONTACTS.DAT',status='replace')
    cycle
  end if
...
do
  ...
  if (all_pas<0) then   ! We wants to calculate each time step
    ...
    if (calcul_coordination         == 1)       call coordination
    if (calcul_anisotropie_contact  == 1)       call anisotropie_contact
...
implicit none
real(kind=8)                             :: mean_contact,mean_neighbor,nb_floating_particle,mean_gap,max_gap,N_active_ctc
real(kind=8)                             :: mean_diameter,cpt_polyg,cpt_disk,cpt_part,Z
integer                                  :: i,cd,an

print*, '---> Coordination'

TAB_RIGID_PARTICLE(:)%nb_neighbor = 0
TAB_RIGID_PARTICLE(:)%nb_ctc = 0
TAB_MESHE_PARTICLE(:)%nb_neighbor = 0
TAB_MESHE_PARTICLE(:)%nb_ctc = 0
mean_gap = 0.
max_gap  = 0.
N_active_ctc = 0
do i=1,nb_ligneCONTACT_PARTICLE
  ...
  if (TAB_CONTACT_PARTICLE(i)%nature=='PLPLx'.or.&
      TAB_CONTACT_PARTICLE(i)%nature=='DKDKx'.or.&
      TAB_CONTACT_PARTICLE(i)%nature=='DKPLx') then
    TAB_RIGID_PARTICLE(cd)%nb_neighbor = TAB_RIGID_PARTICLE(cd)%nb_neighbor + 1
    TAB_RIGID_PARTICLE(an)%nb_neighbor = TAB_RIGID_PARTICLE(an)%nb_neighbor + 1
    TAB_RIGID_PARTICLE(cd)%nb_ctc = TAB_RIGID_PARTICLE(cd)%nb_ctc + TAB_CONTACT_PARTICLE(i)%type
    TAB_RIGID_PARTICLE(an)%nb_ctc = TAB_RIGID_PARTICLE(an)%nb_ctc + TAB_CONTACT_PARTICLE(i)%type

    mean_gap = mean_gap + abs(TAB_CONTACT_PARTICLE(i)%gap)
    max_gap  = max(max_gap,abs(TAB_CONTACT_PARTICLE(i)%gap))
    N_active_ctc = N_active_ctc + 1
  end if

  if (TAB_CONTACT_PARTICLE(i)%nature=='PLJCx'.or.&
      TAB_CONTACT_PARTICLE(i)%nature=='DKJCx') then
    TAB_RIGID_PARTICLE(cd)%nb_neighbor = TAB_RIGID_PARTICLE(cd)%nb_neighbor + 1
    TAB_RIGID_PARTICLE(cd)%nb_ctc = TAB_RIGID_PARTICLE(cd)%nb_ctc + TAB_CONTACT_PARTICLE(i)%type
  ...
  if (TAB_CONTACT_PARTICLE(i)%nature=='DKALp') then
    TAB_RIGID_PARTICLE(cd)%nb_neighbor = TAB_RIGID_PARTICLE(cd)%nb_neighbor + 1
    TAB_MESHE_PARTICLE(an)%nb_neighbor = TAB_MESHE_PARTICLE(an)%nb_neighbor + 1
    TAB_RIGID_PARTICLE(cd)%nb_ctc = TAB_RIGID_PARTICLE(cd)%nb_ctc + TAB_CONTACT_PARTICLE(i)%type
    TAB_MESHE_PARTICLE(an)%nb_ctc = TAB_MESHE_PARTICLE(an)%nb_ctc + TAB_CONTACT_PARTICLE(i)%type
  ...
  if (TAB_CONTACT_PARTICLE(i)%nature=='CLALp') then
    TAB_MESHE_PARTICLE(cd)%nb_neighbor = TAB_MESHE_PARTICLE(cd)%nb_neighbor + 1
    TAB_MESHE_PARTICLE(an)%nb_neighbor = TAB_MESHE_PARTICLE(an)%nb_neighbor + 1
    TAB_MESHE_PARTICLE(cd)%nb_ctc = TAB_MESHE_PARTICLE(cd)%nb_ctc + TAB_CONTACT_PARTICLE(i)%type
    TAB_MESHE_PARTICLE(an)%nb_ctc = TAB_MESHE_PARTICLE(an)%nb_ctc + TAB_CONTACT_PARTICLE(i)%type
...
if (N_active_ctc==0) then
  mean_gap = 0
else
  mean_gap = mean_gap / N_active_ctc
end if

nb_floating_particle= 0
mean_contact = 0.
mean_neighbor= 0.
mean_diameter= 0.
do i=1,nb_RIGID
  if (TAB_RIGID_PARTICLE(i)%nb_ctc<2) then
    nb_floating_particle = nb_floating_particle + 1.
  else
    mean_neighbor = mean_neighbor + TAB_RIGID_PARTICLE(i)%nb_neighbor
    mean_contact  = mean_contact  + TAB_RIGID_PARTICLE(i)%nb_ctc
    mean_diameter = mean_diameter + 2*TAB_RIGID_PARTICLE(i)%Rayon
  end if
end do
do i=1,nb_DEFOR
  if (TAB_MESHE_PARTICLE(i)%nb_ctc<2) then
    nb_floating_particle = nb_floating_particle + 1.
  else
    mean_neighbor = mean_neighbor + TAB_MESHE_PARTICLE(i)%nb_neighbor
    mean_contact  = mean_contact  + TAB_MESHE_PARTICLE(i)%nb_ctc
    !mean_diameter = mean_diameter + 2*TAB_MESHE_PARTICLE(i)%Rayon
  end if
end do

if (nb_floating_particle==nbParticules) then
  mean_contact  = 0
  mean_neighbor = 0
  mean_diameter = 0
else
  mean_diameter = mean_diameter / (nbParticules-nb_floating_particle)
  mean_contact  = mean_contact  / (nbParticules-nb_floating_particle)
  mean_neighbor = mean_neighbor / (nbParticules-nb_floating_particle)
  mean_gap      = mean_gap      / mean_diameter
  max_gap       = max_gap       / mean_diameter
end if
print*,"   -- Mean diameter =",mean_diameter

Z        =  2*N_active_ctc  / (nbParticules-nb_floating_particle)


write(101,'(15(1X,D12.5))') temps,epsilon_q,nb_floating_particle,Z,mean_contact,mean_neighbor,mean_gap,max_gap
end subroutine coordination

!==============================================================================
! Calcul des ansiotropies de contacts signees a partir des tenseurs
!==============================================================================
subroutine anisotropie_contact
implicit none
real(kind=8),dimension(2,2)              :: Fabric

real(kind=8),dimension(2)                :: nik
integer                                  :: i,cd,an,j
real(kind=8)                             :: ac,delta,sigmaone,sigmatwo,cpt


print*, '---> contact  anisotropie'

Fabric   = 0.D0

cpt = 0.D0
do i=1,nb_ligneCONTACT_PARTICLE
  if  (TAB_CONTACT_PARTICLE(i)%nature == 'PLJCx') cycle
  if  (TAB_CONTACT_PARTICLE(i)%nature == 'DKJCx') cycle
  if  (TAB_CONTACT_PARTICLE(i)%nature == 'JCALp') cycle
  if  (TAB_CONTACT_PARTICLE(i)%rn     == 0.D0   ) cycle

  cd      = TAB_CONTACT_PARTICLE(i)%icdent
  an      = TAB_CONTACT_PARTICLE(i)%ianent

  nik(1)  = TAB_CONTACT_PARTICLE(i)%n(1)
  nik(2)  = TAB_CONTACT_PARTICLE(i)%n(2)
...
! For ac
delta = 4 * ((((Fabric(1,1) - Fabric(2,2)) / 2) ** 2) + (Fabric(1,2) * Fabric(2,1)))
sigmaone = (Fabric(1,1) + Fabric(2,2) + (SQRT(delta))) / 2
sigmatwo = (Fabric(1,1) + Fabric(2,2) - (SQRT(delta))) / 2
ac = 2*(sigmaone-sigmatwo)

write(102,'(30(1X,D12.5))') temps,epsilon_q,ac
endsubroutine anisotropie_contact
